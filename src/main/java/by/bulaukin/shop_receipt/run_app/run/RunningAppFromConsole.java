...
public class RunningAppFromConsole  implements RunningApp{
    ...
    public <T> void run(T args) {
        ...
        PrintingViews printingViews = parsingAndViewFactory.createPrintingViews();
        ParsingDataFromRequestsParam dataFromRequestsParam = parsingAndViewFactory.createParsingDataFromRequestsParam();

        try {
            RequestsParsingResult parsingResult = dataFromRequestsParam.parsData(requestsParam);
            ResultChangingEntityToDto result = resultEntityToDtoService.getResult(parsingResult);
            viewServices.createsView(result);
            printingViews.printViews();
        } catch (Exception e){
